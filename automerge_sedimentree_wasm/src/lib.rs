...
impl CommitStore<'static> for WasmSedimentreeAutomerge {
    ...
    fn lookup(&self, digest: Digest<LooseCommit>) -> Result<Option<Self::Node>, Self::LookupError> {
        ...
        // getChangeMetaByHash returns null when the change is not found
        if js_value_should_be_change_meta.is_null() || js_value_should_be_change_meta.is_undefined()
        {
            return Ok(None);
        }

        let Some(_obj) = js_value_should_be_change_meta.dyn_ref::<js_sys::Object>() else {
            // Treat non-object returns as "not found" rather than erroring.
            // The only legitimate returns are null (already handled) or a
            // ChangeMetadata object. A truthy non-object value indicates a
            // degenerate case (e.g., stale externref) with no useful data.
            return Ok(None);
        };
