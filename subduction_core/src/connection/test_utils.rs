...
/// `recv_bytes` / `send_bytes` futures on this side park forever. We
/// implement this by interposing a custom [`Future`] that polls the
/// pause flag on every wake; calling `pause()` triggers a wake via a
/// shared [`event_listener::Event`].
///
/// Pausing one end models a wedge in the remote runtime. The peer's
/// sends still appear to succeed (they enqueue into the channel — that
/// is what `send_bytes` does on the live side) but never get drained,
/// so the wire looks live while no protocol-level progress is possible.
#[derive(Debug, Clone)]
...
impl PausableChannelTransport {
    ...
    /// `recv_bytes` and `send_bytes` calls on this side will park
    /// forever (the futures get woken when the pause fires, then
    /// re-enter and park on a fresh "pending forever").
    pub fn pause(&self) {
...
/// (e.g. asserting that an operation completes well under its
