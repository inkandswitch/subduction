        ...
        ManagedCall<F, H::Message, SendError = <C as Connection<F, H::Message>>::SendError>,
{
    ...
    /// and broadcasts to peers after _every_ commit (O(n²) for n commits),
    ...
    pub async fn add_fragments_batch(
        &self,
        id: SedimentreeId,
        fragments: Vec<(Digest<LooseCommit>, BTreeSet<Digest<LooseCommit>>, Vec<Digest<LooseCommit>>, Blob)>,
    ) -> Result<(), WriteError<F, S, C, H::Message, P::PutDisallowed>> {
        let self_id = self.peer_id();
        let putter = self
            .storage
